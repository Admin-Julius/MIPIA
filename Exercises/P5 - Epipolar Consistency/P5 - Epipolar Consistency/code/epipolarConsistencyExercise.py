...
def load_projections(projection_filename, shape):
    ...
    # TODO
    projections.append(np.load(projection_filename + '0.npy', allow_pickle=True))
    projections.append(np.load(projection_filename + '1.npy', allow_pickle=True))
    # make sure that the projections are returned with correct dimensions
    # TODO
    projections[0] = projections[0].reshape(shape)
    projections[1] = projections[1].reshape(shape)
    return projections
...
def load_geometry(matrices_filename):
    ...
    # TODO
    projection_matrices[0] = projection_matrices[0].reshape((3,4))
    projection_matrices[1] = projection_matrices[1].reshape((3,4))
...
def get_pluecker_coordinates(C0, C1):
    ...
    B = (0, 0, 0, 0, 0, 0)  # 6-tuple, contains all six different entries of the Plücker matrix
    return B
